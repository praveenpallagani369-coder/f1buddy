  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Travel Tracker</h1>
          <p className="text-slate-400 text-sm">Stay within the 5-month limit. Traveling while unemployed still counts toward OPT unemployment days.</p>
        </div>
      ...
      {approachingLimit && !atLimit && (
        <div className="p-4 rounded-xl bg-amber-900/20 border border-amber-800/30 text-sm text-amber-300">
      ...
      <div className="p-3 rounded-lg bg-blue-900/20 border border-blue-800/30 text-xs text-blue-300">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4 text-center">
            <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Total Days Outside ({thisYear})</p>
            <p className={`text-3xl font-bold ${atLimit ? "text-red-400" : approachingLimit ? "text-amber-400" : "text-white"}`}>
              {totalDaysThisYear}
            </p>
            <p className="text-xs text-slate-500 mt-1">of 150 max</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Personal Travel</p>
            <p className="text-2xl font-bold text-white">{personalDays}</p>
            <p className="text-xs text-slate-500 mt-1">days</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Employer-Required</p>
            <p className="text-2xl font-bold text-indigo-400">{employerDays}</p>
            <p className="text-xs text-slate-500 mt-1">days</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <p className={`text-base font-bold mt-1 ${currentlyAbroad ? "text-amber-400" : "text-emerald-400"}`}>
        ...
        <Card className="border-indigo-800/50">
          ...
          <CardContent className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm text-slate-300 mb-1.5">Return Date (blank if still abroad)</label>
                <Input type="date" value={form.returnDate} onChange={(e) => setForm(f => ({ ...f, returnDate: e.target.value }))} />
              </div>
              <div>
                <label className="block text-sm text-slate-300 mb-1.5">Destination Country *</label>
                <Input placeholder="India" value={form.destinationCountry} onChange={(e) => setForm(f => ({ ...f, destinationCountry: e.target.value }))} />
              </div>
              <div>
                <label className="block text-sm text-slate-300 mb-1.5">Travel Type *</label>
                ...
                {form.travelType === "personal" && (
                  <p className="text-xs text-amber-400 mt-1">⚠️ Personal travel while unemployed counts toward your OPT unemployment limit</p>
                )}
              </div>
              <div>
                <label className="block text-sm text-slate-300 mb-1.5">Purpose</label>
            ...
            <div>
              <label className="block text-sm text-slate-300 mb-2">Documents Carried</label>
      ...
      <Card>
        ...
        <CardContent>
          {trips.length === 0 ? (
            <div className="text-center py-8 text-slate-400">No trips logged yet</div>
          ) : (
            <div className="space-y-3">
              {trips.map((t) => (
                <div key={t.id} className="flex items-start justify-between p-4 rounded-lg bg-slate-800/50 border border-slate-800">
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-lg">✈️</span>
                      <p className="text-white font-medium">{t.destination_country}</p>
                    ...
                    <p className="text-sm text-slate-400 mt-0.5">{t.departure_date} → {t.return_date ?? "Present"}</p>
                    <p className="text-xs text-slate-500 mt-0.5 capitalize">{t.purpose} · {t.days_outside} days</p>
                  </div>
