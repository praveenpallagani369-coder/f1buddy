  ...
  return (
    <Card className={isPast ? "opacity-60" : ""}>
      <CardContent className="p-4 text-center">
        ...
        <p className={`text-2xl font-bold ${isPast ? "text-slate-500" : color}`}>
          {isPast ? "Done" : `${days}d`}
        </p>
        <p className="text-xs text-slate-500 mt-1">{format(targetDate, "MMM d, yyyy")}</p>
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-white">H-1B Lottery Timeline</h1>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Countdown targetDate={dates.registrationOpen} label="Registration Opens" color="text-indigo-400" />
        <Countdown targetDate={dates.registrationClose} label="Registration Closes" color="text-amber-400" />
        <Countdown targetDate={dates.selectionNotify} label="Selection Notified" color="text-emerald-400" />
      ...
      <Card className="border-blue-800/40">
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-slate-400">Cap-gap bridges your OPT/STEM OPT authorization until October 1 when H-1B begins, if your EAD expires before then.</p>
          ...
          {h1bFiled && selectedLottery && (
            <div className={`p-4 rounded-xl border ${capGapActive ? "bg-blue-900/20 border-blue-800/30" : "bg-emerald-900/20 border-emerald-800/30"}`}>
              {capGapActive ? (
                <>
                  <p className="font-semibold text-blue-300 mb-1">✅ Cap-Gap Applies to You</p>
                ...
                <>
                  <p className="font-semibold text-emerald-300 mb-1">Your OPT EAD covers you through October 1</p>
                  <p className="text-sm text-emerald-200">Your EAD expires after October 1 ({opt?.ead_end_date}), so cap-gap may not be needed. H-1B starts {format(h1bStartDate, "MMM d, yyyy")}.</p>
                </>
              )}
            </div>
          )}
        ...
        <CardContent>
          <div className="grid sm:grid-cols-2 gap-4 text-sm">
            {[
              ...
              <div key={item.label} className="p-3 rounded-lg bg-slate-800/50 border border-slate-700/50">
        ...
        <CardContent>
          <div className="space-y-3">
            {[
              ...
              <div key={item.title} className="p-3 rounded-lg bg-slate-800/30 border border-slate-700/50">
      ...
      <div className="p-3 rounded-lg bg-slate-800/50 border border-slate-700 text-xs text-slate-500">
